        ...
        try:
            ...
            if similarity_traj:
                ...
                ax_top.plot(
                    ...
                    markersize=3.0,
                    alpha=0.85,
                    label='Similarity to Equilibrium'
                )
                ...
                if conv_analysis.get("converged"):
                    ...
                    if conv_round is not None and conv_round < len(similarity_traj):
                        ax_top.axvline(
                            ...
                            linestyle='--',
                            alpha=0.6,
                            label=f'Convergence (Round {conv_round + 1})'
                        )

                ax_top.legend(loc='lower right', frameon=False, prop={'family': 'Times New Roman', 'size': 12})
            else:
                ax_top.text(
                    0.5, 0.5, 'No similarity trajectory',
            ...
            ax_top.set_ylabel('Jaccard Similarity', color=color_line, fontsize=16, fontfamily='Times New Roman')
            ...
            ax_top.set_title(
                f'Fictitious Play: Similarity and Strategy Evolution ({display_model_name})',
                fontsize=16, fontweight='semibold', color=color_title, fontfamily='Times New Roman'
            )
            ...
            ax_heat.set_xlabel('Round', fontsize=16, fontfamily='Times New Roman', labelpad=14)
            ax_heat.set_ylabel('User ID', fontsize=16, fontfamily='Times New Roman')
            ...
            patch_no_share = mpatches.Patch(color=color_heat_0, ec='#CBD5E1', label='Light = Not Share (0)')
            patch_share = mpatches.Patch(color=color_heat_1, label='Blue = Share (1)')
            patch_theo = mpatches.Patch(color=color_star, label='Theo = Theoretical Eq (1)')
            fig.legend(
